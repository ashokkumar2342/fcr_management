...
//     }
    ...
    {
        try {             
            $rs_districts = DB::select(DB::raw("select * from `districts` order by `name_e`;"));  
            return view('admin.master.districts.index', compact('rs_districts'));
        } catch (Exception $e) {
    ...
    {  
        ...
        $l_code = MyFuncs::removeSpacialChr($request->code);
        $l_name_e = MyFuncs::removeSpacialChr($request->name_english);
        $l_name_h = MyFuncs::removeSpacialChr($request->name_local_language);

        if (!empty($id)) {
            $rs_district = DB::select(DB::raw("update `districts` set `code` ='$l_code', `name_e` = '$l_name_e', `name_l` ='$l_name_h' where `id` = $id limit 1;")); 
            $response=['status'=>1,'msg'=>'District Updated Successfully'];
        }else{ 
            $rs_district = DB::select(DB::raw("insert into `districts` (`code`, `name_e`, `name_l`) values ('$l_code' , '$l_name_e' , '$l_name_h')")); 
            $response=['status'=>1,'msg'=>'District Saved Successfully'];
        }
...
//     }
    ...
    {
        try {
            $Districts=DB::select(DB::raw("select * from `districts`  where `id` = $id limit 1;"));
    ...
    {
       $id = Crypt::decrypt($id);  
       $District = DB::select(DB::raw("delete from `districts` where `id` = $id limit 1;"));
